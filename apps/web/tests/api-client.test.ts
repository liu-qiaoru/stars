...
describe("typed API client", () => {
  test("requests libraries and creates scan jobs with stable routes", async () => {
    fetchMock
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ items: [{ id: "lib-1", name: "Main", root_path: "/media", enabled: true }] }), {
          status: 200,
        }),
      )
      .mockResolvedValueOnce(new Response(JSON.stringify({ job_id: "job-1", status: "queued" }), { status: 200 }));
    ...
    await expect(client.scanLibrary("lib-1")).resolves.toEqual({ job_id: "job-1", status: "queued" });

    expect(fetchMock).toHaveBeenNthCalledWith(1, "http://api.local/libraries", expect.objectContaining({ method: "GET" }));
  ...
  test("posts search requests with media filters and pagination", async () => {
    fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ limit: 12, offset: 24, groups: [] }), { status: 200 }));
    const client = createApiClient({ baseUrl: "http://api.local", fetcher: fetchMock });
  ...
  test("creates, fetches, and confirms agent runs", async () => {
    fetchMock
      .mockResolvedValueOnce(new Response(JSON.stringify({ run_id: "run-1", status: "succeeded" }), { status: 200 }))
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            id: "run-1",
            status: "succeeded",
            prompt: "查找片段",
            tool_calls: [{ tool_call_id: "search-1", name: "search_media", status: "succeeded", summary: "完成搜索" }],
            events: [],
            results: [],
          }),
          { status: 200 },
    ...
    await expect(client.createAgentRun({ prompt: "查找片段", allow_external_vlm: false })).resolves.toEqual({
      run_id: "run-1",
      status: "succeeded",
    });
