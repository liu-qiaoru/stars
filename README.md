...
端到端检索链路需要 **5 个基础进程** 协同工作；开启 Qwen2.5-VL caption indexing 或后续 VLM rerank 时，还需要第 6 个本地 VLM 服务。TypeScript 是主控层，Python 只承担媒体处理和模型推理重任务。
...
│   scan → probe → index → embed → transcribe → ocr → caption → export │
│   从 PostgreSQL jobs 表领取任务，执行媒体/模型重任务             │
│   写 Qdrant points + media_assets + vector_refs                  │
└────────────────────────────────┬─────────────────────────────────┘
                                 │ 可选：LOCAL_VLM_ENABLED=true
                                 ▼
                  Qwen2.5-VL Service :4030 (/caption)
...
> **Python 进程边界**：① `media_agent_worker`（worker，批量索引）；② `media_agent_worker.model_service`（常驻 localhost RPC，供后端搜索时同步生成 query embedding）；③ 可选 `media_agent_worker.vlm_service`（Qwen2.5-VL caption/rerank 服务，默认 `:4030`）。worker、model service 和 VLM service 都只在本机运行。
...
- **Caption retrieval 独立于 OCR/transcript**。Qwen2.5-VL 生成的 caption 写入 `asset_type='caption'`，通过 `caption_text_vectors` 检索并返回 `caption_match`；Phase 15A 不把 caption 纳入 PostgreSQL FTS，不持久缓存视频抽帧。
...
apps/worker-py    Python 媒体与模型 worker（从 Phase 4 起）
  ...
  ├── media_agent_worker/model_service.py  本地常驻 query embedding RPC 服务
  └── media_agent_worker/vlm_service.py    本地 Qwen2.5-VL caption RPC 服务
...
### 5a. 本地 VLM 服务（可选：Qwen2.5-VL Caption）

仅当启用 caption indexing 或后续 VLM rerank 时需要启动：

```bash
pnpm dev:vlm
```

默认监听 `http://127.0.0.1:4030`，提供 `/caption` 和 `/health`。启用 Phase 15A caption retrieval 需要在 `.env` 中打开：
...
Phase 15A 的视频 caption 会临时抽取代表帧，VLM 调用结束后删除临时帧文件；不会把视频帧持久缓存到本地。
...
> 后端 API 默认启用 `JobsCoordinatorService`，每 `JOB_COORDINATOR_INTERVAL_MS` 扫描一次 pending embedding/OCR 工作并创建下游 jobs；可用 `JOB_COORDINATOR_ENABLED=false` 暂停自动协调。
...
-- 查看 caption 生成结果
SELECT id, file_id, left(text_content, 120) AS caption, metadata_json
FROM media_assets
WHERE asset_type = 'caption'
ORDER BY created_at DESC
LIMIT 20;
...
> Server 默认启动 `JobsCoordinatorService`，会自动把 pending `vector_refs` 转成 embedding jobs，并补齐待 OCR jobs。下面的接口主要用于 worker 中断、Qdrant 重建或调试时手动补漏。
...
- 图片或视频片段的 VLM caption 语义：来自 `caption_text_vectors`，`reason` 为 `caption_match`（需开启 `CAPTION_SEARCH_ENABLED=true`）。
...
- **`POST /libraries` 返回 500**：通常是数据库未执行 migration。运行 `pnpm db:migrate` 后重试。
