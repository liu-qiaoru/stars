...
- 核心检索在阶段 8 完成干净基线评测后即可独立验收。多帧能力最后进入阶段 9：阶段 9A 通过、9D 尚未得出结论时默认 `off` 但允许显式试用；只有 9A 和 9D 都通过才默认 `visual + rrf + async`；任一门槛失败或 9D 样本不足都强制 `off`。

### 已选方案：RRF 后异步 Top-K 查询相关多帧复核

本计划曾在讨论中把该设计称为“方案 D”。该字母只保留为历史别名，不能作为需求定义。本文后续统一使用正式名称“RRF 后异步 Top-K 查询相关多帧复核”，其完整含义如下：

1. **执行位置：** 先完成普通检索并得到 RRF 排序，再处理其中排名最靠前的 K 个视频场景；默认 `K=3`。它不是新的召回通道，不负责寻找 RRF 没有召回的场景。
2. **执行方式：** Server 完成 RRF 后先在 PostgreSQL 创建 `verify_multi_frame_search` 异步任务，再把 RRF 结果和任务 ID 一起返回给 Web，但不等待任务执行。Python Worker 稍后领取任务、抽帧并调用 VLM；Web 通过任务 ID 轮询结果，因此普通搜索只等待一次很短的任务入库，不等待 VLM 推理。
3. **模型输入：** VLM 只接收用户的中文原查询和从候选场景生成的、按时间排序的最多 12 张图片；不接收完整 MP4、英文译文、查询扩展词、Caption 或语音转录。
4. **抽帧组成：** 每个候选最多使用 5 张全场景均匀覆盖帧、5 张 SigLIP2 最佳命中时间附近的 0.5 秒加密帧和 2 张画面变化峰值帧。Caption-only 候选没有 SigLIP2 最佳命中时间，只使用均匀帧和画面变化峰值帧。
5. **模型输出：** Ollama `qwen2.5vl:7b` 只生成 `relevance: 0 | 1 | 2`、`matched_constraints`、`missing_constraints` 和 `reason`；候选 ID、时间戳、图片哈希和模型版本由服务代码附加，不能信任模型回显。
6. **排序边界：** 只有 Top-K 视频全部成功复核才应用新顺序；VLM 不能删除候选、加入候选或改变图片位置，只能在这些视频原来占据的位置之间按相关等级重排。同等级保持原 RRF 顺序。
7. **失败行为：** 任一候选抽帧失败、超时、响应格式错误、模型版本不一致或索引发生变化时，整组保留原 RRF 排序，并向 Web 返回可见的结构化错误；部分成功结果不得参与排序。
8. **上线门槛与功能状态：** 该能力有四种生命周期情形，对应 `unavailable | experimental_opt_in | default_on` 三种可用性状态。阶段 9A 运行前或未通过时为 `unavailable`，不能创建复核任务；9A 通过但 9D 尚未得出结论时为 `experimental_opt_in`，只允许用户显式开启以便试用和完成对照评测，默认仍为 `off`；9D 通过后为 `default_on`，默认使用 `async`，用户仍可关闭；9D 失败或样本不足时回到 `unavailable`，不能用质量回退或证据不足的结果修改生产排序。

- 2026-07-15 的 Transformers Qwen2.5-VL-7B 完整 MP4 实验未通过 32 GiB 本机性能门槛，因此永久停止该方案，不建设 `/verify-video`，也不把历史失败改写成成功。
- 2026-07-17 明确采用上述“RRF 后异步 Top-K 查询相关多帧复核”，但把它放到核心检索完成之后：阶段 9A 通过后选项才以实验状态可用，阶段 9D 也通过后才默认开启，用户始终可关闭；9D 失败或样本不足则重新禁用。这个决策是对原计划“不得自动切换多帧方案”的显式重新决策。
...
**目的：** 核心检索完成后，再验证“RRF 后异步 Top-K 查询相关多帧复核”能否在同一台 32 GiB 机器上，以有限数量的查询相关图片改善 Top-K 判断。阶段 9A 只做隔离试验，不修改生产排序。
...
- 删除约 15 GiB 的 Hugging Face Qwen2.5-VL-7B 权重缓存仍是独立磁盘清理，必须由用户另行明确确认；采用本多帧复核方案本身不授权删除。`transformers` Python 库仍需保留给 SigLIP2 和 Caption 文本嵌入。
...
阶段 9A 通过后、阶段 9D 尚未得出结论时，才允许用户显式请求 `multi_frame_verification_mode=async`，默认仍为 `off`；阶段 9D 通过后默认 `async`，9A 未通过、9D 已失败、9D 样本不足或后端不可用时强制为 `off`。流程：
...
→ PostgreSQL 提交任务后，Server 返回初始结果和 verification_job_id
...
- 阶段 9A 未通过、阶段 9D 已失败或样本不足、`spoken/all`、非 RRF 或后端不可用时禁用多帧复核并解释具体原因。
...
- 在运行 C 前冻结 `multi-frame-gate-v1` 评测配置，至少包含 50 条实际进入多帧复核的视频查询，短动作、人物关系和环境约束每组至少 8 条；不足时记为“样本不足”，可用性状态回到 `unavailable`，不得继续创建生产复核任务。这些数量是最低覆盖要求，不代表已经具有普遍统计结论；补足样本后可以重新运行阶段 9D。
- 在阶段 8 的 A/B 之外增加 C：
...
- 多帧分支最终必须满足下面两个互斥结果之一，任一结果都不阻止 SigLIP2 + RRF 主线完成：
  ...
  - **不可用：** 阶段 9A 未通过，或 9A 通过但阶段 9D 失败或样本不足。报告保留，功能开关和 Web 选项明确禁用，普通搜索不创建 `verify_multi_frame_search`，页面说明当前展示 RRF 排序；已经实现的实验或服务代码不能通过默认配置进入生产调用链。样本不足时可以补充标注后重新评测，但重新通过前不得恢复生产入口。
