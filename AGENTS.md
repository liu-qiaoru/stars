...
**File indexed status follows the first searchable vector.** After a worker successfully writes any active vector ref to Qdrant, `mark_vector_ref_indexed(point_id)` updates both `vector_refs.status='indexed'` and its active `media_files.index_status='indexed'` in the same transaction. A file does not wait for every pending/failed ref before contributing to library `indexed_count`; migration `0002_backfill_indexed_media_files.sql` repairs historical rows.
...
- *Caption indexing*: with `CAPTION_INDEXING_ENABLED=true` and `LOCAL_VLM_ENABLED=true`, image assets use single-image `caption-v1`; each video segment uses `scene-caption-v2` and sends 1–`SCENE_CAPTION_MAX_FRAMES` ordered frames from the same scene to Qwen2.5-VL. The worker records source asset IDs/times/model/prompt provenance, writes `caption_text_vectors`, and cleans all temporary images on success or failure.
- *Video scene retrieval*: scene detection windows are capped by `SCENE_MAX_SECONDS` (default 30s), including stable-ID fallback windows. Every active `video_segment` must have at least one same-scene `video_frame`; only frames create new visual vectors. Search keeps raw frame groups, collapses top-level video candidates by `(file_id, scene_id)` MaxSim, and gets boundaries from PostgreSQL. `VIDEO_SEGMENT_SEARCH_ENABLED` is migration-only and must be disabled only after `GET /jobs/video/reindex-readiness` is visually ready.
...
**Library file browsing is lazy and paginated.** `GET /libraries/:id/media` returns active files ordered by `(relative_path, id)`, defaults to 25 rows, and caps `limit` at 100. The web library cards stay collapsed without fetching files, append pages with “加载更多”, preserve loaded rows on collapse/error, and redirect successful scan creation to `/jobs` where auto-refresh shows progress. AppShell uses pathname-aware `aria-current='page'` for active navigation and Ask.
