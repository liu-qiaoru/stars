...
## 2026-06-27：Hybrid retrieval 方案必须先定义合并后语义

规划跨来源检索和 reranking 时，不能只写“合并结果”。必须在实现前定义合并后 result 的 identity（代表 `asset_id`、`merged_asset_ids`、跳转依据）、source key 命名、同 source 多次命中合并规则、分页时机、overfetch 策略、时间范围 null 语义、groups 兼容策略和分数归一化规则。尤其不要用 per-query min-max 把单一低分结果放大到满分；没有历史校准时应优先使用来源内稳定映射，并用测试覆盖低分单来源不压过多信号候选。`source_scores` 如果保留原始分数，必须写明不能跨 source 直接比较；multi-signal bonus 这类启发式权重要标明为可调经验值。

## 2026-06-24：工具清单类文档要先列范围再写计划

当用户要求整理项目中使用的工具时，应先给出候选工具列表和分类范围，让用户确认哪些算“项目工具”，再写入任务计划或正式文档。不要先进入计划确认而跳过用户最需要的工具范围梳理。
...
## 2026-06-16：新增运行时进程必须同步 README 启动流程

每引入一个必须常驻的本地进程，例如 Python model service 或 Python worker，README 的“启动服务”必须同步补齐命令、用途、缺失时的影响，以及它是否依赖外部 API。不能只在实现文档或代码里存在，否则用户会以为 server/web 已足够完成端到端索引。

## 2026-06-16：解释 Python 逻辑要按非 Python 读者展开

用户阅读 Python 代码有成本。后续回答涉及 Python worker、repository、扫描、索引、OCR、转写等逻辑时，不能只说函数名或简单概括，需要用接近伪代码的方式拆出输入、关键变量、分支判断、数据库读写、返回值和后续 job 影响，并明确说明每一步为什么存在。
## 2026-07-12：NestJS ESM Controller 必须验证真实 DI

- 本项目的 TypeScript/ESM 配置不能假设构造器类型一定产生可用的运行时注入元数据。Controller 注入 class provider 时沿用项目既有模式，显式使用 `@Inject(ProviderClass)`。
- 只手工 `new Controller(fakeService)` 的测试无法发现缺失注入装饰器。新增 Controller 时至少有一个测试通过 Nest `TestingModule` 创建实例，再调用公开入口验证 provider 实际注入。
- 页面 SSR 出现 500 时，先分别请求前端页面和对应后端 API，再从后端完整异常栈定位；不要把 API 500 当成前端渲染错误，也不要在未复测前把问题归因于 migration。

## 2026-07-12：异步表单处理前保存 DOM 引用

- React 表单事件的 `currentTarget` 只保证在同步事件处理阶段可用。异步请求完成后再访问 `event.currentTarget.reset()` 可能得到 `null`。
- 在第一次 `await` 之前保存 `const form = event.currentTarget`，后续读取 `FormData` 和重置都使用这个稳定引用。
- 表单回归测试必须等待异步请求完成，并断言成功状态、输入清空且页面没有 error alert；仅断言 API 被调用无法捕获事件生命周期错误。

## 2026-07-12：评测目标必须通过领域对象点选

- `file_id` 和 `scene_id` 是系统内部身份，不应要求标注者查询数据库后手工复制。
- 指定目标查询应从素材库文件开始：按相对路径服务端筛选，图片直接选择，视频读取已有 `video_segment` 并选择带稳定 `scene_id` 的时间段。
- 目标选择必须展示事实内容：图片显示缩略图，视频列表显示预览，scene 使用带准确 `#t=start,end` 时间片段的播放器；只有时间文字不足以可靠判断场景。
- 没有稳定 `scene_id` 的视频要明确显示不可选择，不能用临时时间窗冒充可复用的评测实体。
- 创建指定目标查询时优先提供 seeded 随机样本，标注者先看原始媒体再描述查询；抽样接口不得返回 Caption、OCR 或 Transcript，避免模型文本污染人工查询措辞。
- 随机批次应控制媒体多样性：图片与视频尽量平衡，同一视频最多一个 scene；相同 seed 保持顺序稳定，只有用户主动“换一批”才改变样本。
